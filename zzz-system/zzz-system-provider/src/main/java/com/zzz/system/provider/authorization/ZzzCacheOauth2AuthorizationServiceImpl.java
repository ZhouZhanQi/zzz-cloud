...
import com.zzz.system.api.model.constants.ZzzOauthConstant;
...
public class ZzzCacheOauth2AuthorizationServiceImpl implements OAuth2AuthorizationService {
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        AssertUtils.checkArgument(StrUtil.isNotBlank(token), "token cannot be empty");
        AssertUtils.checkNotNull(tokenType, "tokenType cannot be empty");
    ...
    private String buildCacheKey(String type, String id) {
        return String.format("%s::%s::%s", ZzzOauthConstant.AUTHORIZATION, type, id);
    }
}
